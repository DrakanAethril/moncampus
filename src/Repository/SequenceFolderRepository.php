...
/**
 * @extends ServiceEntityRepository<SequenceFolder>
 */
class SequenceFolderRepository extends ServiceEntityRepository
{
    public function __construct(ManagerRegistry $registry)
    {
        parent::__construct($registry, SequenceFolder::class);
    }
    ...
    {
        /** @var list<SequenceFolder> $folders */
    ...
    /**
     * The sub-folders of one folder, by name - the rows the listing draws before the séquences.
     *
     * @return list<SequenceFolder>
     */
    public function findChildren(User $owner, ?SequenceFolder $parent): array
    {
        ...
        /** @var list<SequenceFolder> $children */
    ...
    /**
     * The names already taken among a folder's siblings - what App\Service\SequenceFolderTree needs
     * to answer « Nouveau dossier », « Nouveau dossier (2) ».
     *
     * @return list<string>
     */
    public function siblingNames(User $owner, ?SequenceFolder $parent, ?int $exceptId = null): array
    {
        $builder = $this->createQueryBuilder('f')
            ->select('f.name')
            ->where('f.owner = :owner')
     ...
     * @return list<SequenceFolder>
     */
    public function findSubtree(SequenceFolder $folder): array
    {
        /** @var list<SequenceFolder> $subtree */
