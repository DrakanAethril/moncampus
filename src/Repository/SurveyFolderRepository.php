...
/**
 * @extends ServiceEntityRepository<SurveyFolder>
 */
class SurveyFolderRepository extends ServiceEntityRepository
{
    public function __construct(ManagerRegistry $registry)
    {
        parent::__construct($registry, SurveyFolder::class);
    }
    ...
    /**
     * The sub-folders of one folder, by name - the rows the listing draws before the models.
     *
     * @return list<SurveyFolder>
     */
    public function findChildren(User $owner, ?SurveyFolder $parent): array
    {
        ...
        /** @var list<SurveyFolder> $children */
    ...
    /**
     * The names already taken among a folder's siblings - what App\Service\Survey\SurveyFolderTree
     * needs to answer « Nouveau dossier », « Nouveau dossier (2) ».
     *
     * @return list<string>
     */
    public function siblingNames(User $owner, ?SurveyFolder $parent, ?int $exceptId = null): array
    {
        $builder = $this->createQueryBuilder('f')
            ->select('f.name')
            ->where('f.owner = :owner')
     ...
     * @return list<SurveyFolder>
     */
    public function findSubtree(SurveyFolder $folder): array
    {
        /** @var list<SurveyFolder> $subtree */
