 ...
 *
 * None of them is visible from the screen once it has settled, and the ones a link can ask for are
 * exactly the ones no other test can see: `?date=` says « that day, chronological », and the ‹ ›
 * arrows carry a `?week=` that has to outrank it.
 *
 * A class is deliberately NOT one of them any more: it is the route
 * (`/programs/{id}/lesson-log`), and a scoped screen has one class, so there is nothing left for
 * this service to choose between.
 */
class LessonLogPeriodBoardTest extends TestCase
{
    ...
    {
        self::assertSame(LessonLogPeriodBoard::MODE_CLASS, $this->board->viewMode(null, null, null));
    }

    public function testArrivingWithADateOpensTheChronologicalList(): void
    {
        self::assertSame(LessonLogPeriodBoard::MODE_CHRONOLOGICAL, $this->board->viewMode(null, '2026-09-02', null));
    }

    public function testAnExplicitModeWinsOverEverything(): void
    {
        self::assertSame(LessonLogPeriodBoard::MODE_CLASS, $this->board->viewMode('class', '2026-09-02', 'chronological'));
        self::assertSame(LessonLogPeriodBoard::MODE_CHRONOLOGICAL, $this->board->viewMode('chronological', null, 'class'));
    }

    public function testTheRememberedModeOnlyAnswersWhenNothingElseDoes(): void
    {
        self::assertSame(LessonLogPeriodBoard::MODE_CHRONOLOGICAL, $this->board->viewMode(null, null, 'chronological'));
        // A day in the URL is a fresh instruction and outranks what the last visit left behind.
        self::assertSame(LessonLogPeriodBoard::MODE_CHRONOLOGICAL, $this->board->viewMode(null, '2026-09-02', 'class'));
    }

    public function testAnUnreadableModeIsIgnoredRatherThanShownEmpty(): void
    {
        self::assertSame(LessonLogPeriodBoard::MODE_CLASS, $this->board->viewMode('sideways', null, null));
        self::assertSame(LessonLogPeriodBoard::MODE_CLASS, $this->board->viewMode(null, null, 'sideways'));
    }
    ...
    {
        self::assertSame(11, $this->board->selectedSession($this->rows(), null, null));
    }

    public function testAnAskedForSeanceIsPreviewed(): void
    {
        self::assertSame(13, $this->board->selectedSession($this->rows(), 13, null));
    }
    ...
    public function testASeanceOutsideThePeriodFallsBackToTheFirstOne(): void
    {
        ...
        self::assertSame(11, $this->board->selectedSession($this->rows(), 99, null));
    }

    public function testADateSelectsTheFirstSeanceOfThatDay(): void
    {
        self::assertSame(13, $this->board->selectedSession($this->rows(), null, '2026-09-02'));
    }
    ...
    public function testADateWithoutSeanceSelectsNothing(): void
    {
        ...
        self::assertNull($this->board->selectedSession($this->rows(), null, '2026-09-01'));
    }

    public function testAnEmptyPeriodSelectsNothing(): void
    {
        self::assertNull($this->board->selectedSession([], null, null));
    }

    // --- What is unfolded ---

    public function testTheClassCarryingThePreviewedSeanceOpens(): void
    {
        self::assertSame(2, $this->board->openClass($this->rows(), 12));
    }

    public function testWithNothingPreviewedTheFirstClassOpens(): void
    {
        self::assertSame(1, $this->board->openClass($this->rows(), null));
        self::assertNull($this->board->openClass([], null));
    }
