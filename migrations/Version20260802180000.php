...
final class Version20260802180000 extends AbstractMigration
{
    ...
    {
        return "Importe les cahiers de texte des séances depuis l'export Notion (100 séances, 17 séquences).";
    }
    ...
    public function postUp(Schema $schema): void
    {
        ...
        foreach (self::ENTRIES as [$sequence, $seance, $contenu]) {
            $trouvees = (int) $this->connection->fetchOne(
                'SELECT COUNT(*) FROM seance_template st INNER JOIN sequence_template sq ON sq.id = st.sequence_template_id WHERE sq.titre = ? AND st.titre = ?',
                [$sequence, $seance],
            );
    ...
    public function down(Schema $schema): void
    {
        foreach (self::ENTRIES as [$sequence, $seance, $contenu]) {
            $this->addSql(
                'UPDATE seance_instance si INNER JOIN seance_template st ON st.id = si.source_template_id INNER JOIN sequence_template sq ON sq.id = st.sequence_template_id SET si.cahier_de_texte_description = NULL WHERE sq.titre = ? AND st.titre = ? AND si.cahier_de_texte_description = ?',
                [$sequence, $seance, $contenu],
            );
            $this->addSql(
                'UPDATE seance_template st INNER JOIN sequence_template sq ON sq.id = st.sequence_template_id SET st.cahier_de_texte_description = NULL WHERE sq.titre = ? AND st.titre = ? AND st.cahier_de_texte_description = ?',
                [$sequence, $seance, $contenu],
            );
        }
    }
