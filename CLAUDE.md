...
has actually answered 200, since `deploy-prod.sh` returns while the container is still installing
and the site 502s for a while — when the deploy starts, when it succeeds (the summary, never the
