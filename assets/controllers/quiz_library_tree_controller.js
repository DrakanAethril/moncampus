...
// The quiz library's rail - the file library's tree controller (file_library_tree_controller.js),
// reduced to the one gesture this feature has and widened to the two things that can be dropped.
//
// What is dragged is a **row of the listing**, which is outside this element, or a folder of the rail
// itself; what receives it is always a folder - or the rail's root entry, which files a quiz back out
...
export default class extends Controller {
    ...
    connect() {
        ...
        this.screen = this.element.closest('.cm-qlib') ?? document;
    ...
    toggleBranch(event) {
        ...
        const node = event.currentTarget.closest('.cm-qlib__node');
    ...
    // A folder of the rail, a folder row of the listing, or a quiz row: one handler, because "what is
    // being dragged" is answered by the attribute it carries and not by where it came from.
    dragStart(event) {
        const source = event.target.closest('[data-quiz-id], [data-folder-id]');

        if (!source) return;

        const quizId = source.dataset.quizId;
        ...
        if (!quizId && !folderId) return;

        this.dragged = quizId ? { kind: 'quiz', id: quizId } : { kind: 'folder', id: folderId };
    ...
    dragOver(event) {
        const node = event.target.closest('.cm-qlib__node');
    ...
    async move(parentId) {
        const template = this.dragged.kind === 'quiz' ? this.quizMoveUrlTemplateValue : this.folderMoveUrlTemplateValue;
